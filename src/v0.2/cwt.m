...
%   double-precision real- or complex-valued vector or a single-variable
%   regularly sampled timetable and must have at least 4 samples. The CWT
%   is obtained using the analytic Morse wavelet with the symmetry
%   parameter (gamma) equal to 3 and the time-bandwidth product equal to
%   60. The CWT uses 10 voices per octave. The minimum and maximum scales
%   are determined automatically based on the wavelet's energy spread in
%   frequency and time. If X is real-valued, WT is a 2-D matrix where each
%   row corresponds to one scale. The column size of WT is equal to the
%   length of X. If X is complex-valued, WT is a 3-D matrix, where the
%   first page is the CWT for the positive scales (analytic part or
...
%   first element of FrequencyLimits specifies the lowest peak passband
...
if IsTimeTable
    ...
    [TTable.times,TTable.units,TTable.convertFunc] = ...
        getDurationandUnits(TTable.SampleTimes);
...
if isempty(fb.SamplingPeriod)
    ...
    [dt,Units,dtfunc] = getDurationandUnits(fb.SamplingPeriod);
...
elseif nargout == 0 && (~isempty(fb.SamplingFrequency) || tfnormfreq)
    [FourierFactor,sigmaT] = wavelet.internal.wavCFandSD(fb.Wavelet,ga,be);
...
if ~antiAnalytic
    ...
    hold(AX,'on');
    hs = surface('Parent',AX,...
        'XData',[min(t) max(t)],'YData',[min(period) max(period)],...
        'CData',abs(wt), 'ZData', zeros(2,2), ...
...
elseif antiAnalytic
    ...
    hold(AX(1),'on');
    newplot(AX(1));
    hs1 = surface('Parent',AX(1),...
        'XData',[min(t) max(t)],'YData',[min(period) max(period)],...
        'CData',abs(wt(:,:,1)), 'ZData', zeros(2,2), ...
        'CDataMapping','scaled', 'FaceColor','texturemap', 'EdgeColor', 'none');
    ...
    % interactivity is restored
    hold(AX(2),'on');
    hs2 = surface('Parent',AX(2),...
        'XData',[min(t) max(t)],'YData',[min(period) max(period)],...
        'CData',abs(wt(:,:,2)), 'ZData', zeros(2,2), ...
        'CDataMapping','scaled', 'FaceColor','texturemap', 'EdgeColor', 'none');
...
if ~antiAnalytic
    ...
    hold(AX,'on');
    hs = surface('Parent',AX,...
        'XData',[min(T) max(T)],'YData',[max(freq) min(freq)],...
        'CData',abs(wt), 'ZData', zeros(2,2), ...
        'CDataMapping','scaled', 'FaceColor','texturemap', 'EdgeColor', 'none');
...
elseif ~isempty(params.Ts) && ~strcmpi(params.WAV,'morse')
    ...
    [~,~,convertFunc] = getDurationandUnits(params.Ts);
...
if isduration(fs)
    times = ttable.times;
    DT = mean(diff(times));
    [sp,spunits] = getDurationandUnits(fs);
...
% Find the closest x and y points to this position.
[~,xIdx] = min(abs(varargin{1} - pos(1)));
[~,yIdx] = min(abs(varargin{2} - pos(2)));

thisTime = varargin{1}(xIdx);
thisPeriod = varargin{2}(yIdx);
...
cVal = event_obj.Target.CData(yIdx, xIdx);
...
if Nvarargin < 3
    ...
    thisTime = datestr(varargin{1}(xIdx));
end
thisFreq = varargin{2}(yIdx);
if Nvarargin < 3
    output_txt{1} = [getString(message('Wavelet:cwt:Time')) ': ', num2str(thisTime, 4)];
else
...
% Reset the data cursor manager to set the SnapToDataVertex to 'on' when
% CWT surface is destroyed.
