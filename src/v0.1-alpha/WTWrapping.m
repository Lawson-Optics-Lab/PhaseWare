function [phase2, phase1, dim_out] = WTWrapping(I0,NS, R,HLPF,RemoveCarrier,dim_in)
% wrapping an open fringe pattern with wavelet transform 
...
for k=1:M
    y = I0(k,:);
    c1=cwt(y,1:1:32, 'cmor1-1.5');    A1=abs(c1);
    psi = angle(c1);
    [~,row_index1]=max(A1);
    for m=1:N
        phase1(k,m)=psi(row_index1(1,m),m);
    end
end
if RemoveCarrier==1
