...
function inOut = UnwrappingGhigliaRomero(imIn, weight)
    if (nargin < 2) % unweighted phase unwrap
        % get the wrapped differences of the wrapped values
        dx = [zeros([size(imIn,1),1]), wrapToPi(diff(imIn, 1, 2)), zeros([size(imIn,1),1])];
        dy = [zeros([1,size(imIn,2)]); wrapToPi(diff(imIn, 1, 1)); zeros([1,size(imIn,2)])];
        rho = diff(dx, 1, 2) + diff(dy, 1, 1);
        
        % get the result by solving the poisson equation
        inOut = solvePoisson(rho);
        
    else % weighted phase unwrap
        % check if the weight has the same size as psi
        if (~all(size(weight) == size(imIn)))
        ...
        dx = [wrapToPi(diff(imIn, 1, 2)), zeros([size(imIn,1),1])];
        dy = [wrapToPi(diff(imIn, 1, 1)); zeros([1,size(imIn,2)])];
        ...
        WWdx2 = [zeros([size(imIn,1),1]), WWdx];
        WWdy2 = [zeros([1,size(imIn,2)]); WWdy];
        ...
        while (~all(rk == 0))
            ...
            inOut = inOut + alphak * pk;
            rk = rk - alphak * Qpk;
            
            % check the stopping conditions
            if ((k >= numel(imIn)) || (norm(rk(:)) < eps * normR0)) break; end;
        end
    end
end
